package org.emergent.maven.cloud.gcs.plugin.download;
...
import org.emergent.maven.cloud.gcs.StorageFactory;
import org.emergent.maven.cloud.gcs.plugin.PrefixKeysIterator;
...
public class GCSDownloadMojo extends AbstractMojo {
  ...
  public GCSDownloadMojo(String bucket, List<String> keys, String downloadPath)
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    List<Iterator<Blob>> prefixKeyIterators =
        keys.stream()
  ...
  private void downloadFile(Blob blob) {
    LOGGER.log(
        Level.INFO, "Downloading from bucket " + blob.getBucket() + " with key " + blob.getName());
    ...
    if (isDirectory(blob)) {
      LOGGER.log(
          Level.INFO, "Bucket " + blob.getBucket() + " key " + blob.getName() + " is as directory");
      return;
    }
  ...
  /**
   * Due to blob.isDirectory is not working as expected will have to check if there are more than
   * two files
   *
   * @param blob
   * @return
  ...
  private final boolean isDirectory(Blob blob) {
    Iterator<Blob> blobs =
        storage.list(bucket, Storage.BlobListOption.prefix(blob.getName())).getValues().iterator();
