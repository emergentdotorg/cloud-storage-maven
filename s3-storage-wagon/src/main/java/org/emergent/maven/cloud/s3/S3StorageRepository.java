...
import org.emergent.maven.cloud.resolver.KeyResolver;
import org.emergent.maven.cloud.s3.utils.S3Connect;
import org.emergent.maven.cloud.transfer.TransferProgress;
import org.emergent.maven.cloud.transfer.TransferProgressFileInputStream;
import org.emergent.maven.cloud.transfer.TransferProgressFileOutputStream;
...
public class S3StorageRepository {
  ...
  public S3StorageRepository(
      String bucket, String baseDirectory, PublicReadProperty publicReadProperty) {
    this.bucket = bucket;
    this.baseDirectory = baseDirectory;
    this.publicReadProperty = publicReadProperty;
  }
  ...
  public void copy(String resourceName, File destination, TransferProgress transferProgress)
    ...
    try {
      ...
      destination
          .getParentFile()
          .mkdirs(); // make sure the folder exists or the outputStream will fail.
      try (OutputStream outputStream =
              new TransferProgressFileOutputStream(destination, transferProgress);
  ...
  public void put(File file, String destination, TransferProgress transferProgress)
    ...
    try {
      try (InputStream inputStream = new TransferProgressFileInputStream(file, transferProgress)) {
        PutObjectRequest putObjectRequest =
            new PutObjectRequest(bucket, key, inputStream, createContentLengthMetadata(file));
  ...
  public List<String> list(String path) {
    ...
    ObjectListing objectListing =
        amazonS3.listObjects(new ListObjectsRequest().withBucketName(bucket).withPrefix(key));
