...
package org.emergent.maven.cloud.s3.plugin.upload;
...
public class S3UploadMojo extends AbstractMojo {
  ...
  /**
   * If the path is a file then a file shall be uploaded. If the file specified is a directory then
   * the directory shall be uploaded using prefix and the rest files shall be uploaded recursively
   *
   * @param bucket
   * @param path
   * @param key
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    if (bucket == null) {
      throw new MojoExecutionException(
          "You need to specify a bucket for the s3-upload goal configuration");
    }
  ...
  private String generateKeyName(String fullFilePath) {
    ...
    if (key != null) {
      ...
      final String filePathToAppend =
          clearFilePath.startsWith("/") ? clearFilePath.replaceFirst("/", "") : clearFilePath;
